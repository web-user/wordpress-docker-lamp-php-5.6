...
class WPBakeryShortCode_VC_Raw_html extends WPBakeryShortCode {
	...
	protected function content ( $atts , $content = null , $helper = false ){
        ...
        $css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'raw-code '.$el_class, $this->settings['base']);
        $output .= "\n\t".'<div class="'.$css_class.'">';
        $output .= "\n\t\t".'<div class="code-wrapper">';
        $output .= "\n\t\t\t".$content;
        $output .= "\n\t\t".'</div> '.$this->endBlockComment('.code-wrapper');
        $output .= "\n\t".'</div> '.$this->endBlockComment('.raw-code');
        return $output;
	}
}
