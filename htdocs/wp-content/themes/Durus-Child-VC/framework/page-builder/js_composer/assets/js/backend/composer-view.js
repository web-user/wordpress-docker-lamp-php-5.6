...
(function ($) {
        ...
        changeShortcodeParams:function (model) {
            ...
            if (_.isArray(settings.params)) {
                _.each(settings.params, function (p) {
					...
					if ($wrapper.children('.' + p.param_name).is('span.wpb_vc_icon')) {
					var icon = $wrapper.children('[name=' + p.param_name + ']');
						  $.ajax({
                          type:'POST',
                          url:window.ajaxurl,
                          data:{
                            action:'wpb_icon_code',
                            content: value
                          },
                          dataType:'html',
                          context:this
                    ...
                    else if ($wrapper.children('.' + p.param_name).is('div, h1,h2,h3,h4,h5,h6, span, i, b, strong, button')) {
    ...
    var ElementBlockView = vc.element_block_view = Backbone.View.extend({
        ...
        createElement:function (e) {
			...
			else if (this.container.$el.is('.wpb_vc_section_container')){
				 if ($button.data('element') == 'vc_row') {
    ...
    var VisualComposer = Backbone.View.extend({
        ...
        views:{},
        events:{
            "click #wpb-add-new-section":'createSection',
			"click #wpb-add-new-double-section":'createDoubleSection',
        ...
        addAll:function () {
            ...
            if (this.$content.find('> [data-element_type]:not(.wpb_vc_section , .wpb_vc_double_section , .wpb_vc_row )').length > 0) {
        ...
        setDraggable:function () {
            $('#wpb-add-new-element, #wpb-add-new-section , #wpb-add-new-double-section').draggable({
                helper:function () {
            ...
            this.$content.droppable({
                greedy:true,
                accept:".dropable_el,.dropable_section,.dropable_double_section",
        ...
        dropButton:function (event, ui) {
            if (ui.draggable.is('#wpb-add-new-element')) {
                this.addElement();
            } else if (ui.draggable.is('#wpb-add-new-section')) {
                this.createSection();
            }
			 else if (ui.draggable.is('#wpb-add-new-double-section')) {
                this.createDoubleSection();
            }
        },
        ...
        createSection:function () {
            var section = Shortcodes.create({shortcode:'vc_section'});
			 Shortcodes.create({shortcode:'vc_section_container', params:{width:'1/1'}, parent_id:section.id, root_id:section.id });
            return section;
        },
		 /**
         * Create Double Section
         */
        createDoubleSection:function () {
            var section = Shortcodes.create({shortcode:'vc_double_section'});
			 Shortcodes.create({shortcode:'vc_double_section_container', params:{width:'1/2'}, parent_id:section.id, root_id:section.id });
			 Shortcodes.create({shortcode:'vc_double_section_container', params:{width:'1/2'}, parent_id:section.id, root_id:section.id });
            return section;
        },
        /**
        ...
        updateRowsSorting:function () {
            _.defer(function (app) {
                var $rows = app.$content.find('> .wpb_vc_row , > .wpb_vc_section , > .wpb_vc_double_section');
                $rows.each(function () {
        ...
        setSortable:function () {
            ...
            $('.wpb_main_sortable').sortable({
                ...
                // cursorAt: { left: 10, top : 20 },
                cursor:"move",
                items:"> .wpb_vc_row , > .wpb_vc_section , > .wpb_vc_double_section", // wpb_sortablee
                handle:'.controls_row',
                distance:0.5,
