   ...
   vc.builder = {
    ...
    var VisualComposer = vc.visualComposerView = Backbone.View.extend({
        ...
        initialize:function () {
	        ...
	        Shortcodes.bind('add', vc.atts.addShortcodeIdParam, this); // update vc_grid_id on shortcode adding
        ...
        setSortable:function () {
            ...
            $('.wpb_column_container').sortable({
                ...
                stop:function (event, ui) {
	                ...
	                var is_container = _.isObject(vc.map[tag]) && ( ( _.isBoolean(vc.map[tag].is_container) && vc.map[tag].is_container === true ) || !_.isEmpty(vc.map[tag].as_parent ) );
                    if (is_container && !(allowed_container_element === true || allowed_container_element === ui.item.data('element_type').replace(/_inner$/, ''))) { // && ui.item.hasClass('wpb_container_block')
                        $(this).sortable('cancel');
                    }
