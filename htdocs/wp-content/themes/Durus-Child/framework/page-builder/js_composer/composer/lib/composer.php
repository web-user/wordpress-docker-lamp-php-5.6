...
if (!class_exists('WPBakeryVisualComposer')) {
    class WPBakeryVisualComposer extends WPBakeryVisualComposerAbstract {
       ...
       ---------------------------------------------------------- */
		...
		public function iconCodeJavascript_callback(){
			global $ss_social , $ss_air , $fa_icons , $uploaded_icons , $brad_data;
			$icon_code = $this->post( 'content' );
			if( $icon_code != ''){
		    $icon = explode ("|", $icon_code);
		    $icon_type = $icon[1];
		    $icon_value = $icon[0];
		    if($icon_type == 'ss-air'){
			   echo '<i class="ss-air '.$ss_air[$icon_value].'"></i>';
		    }
			else if($icon_type == 'uploaded'){
			   echo '<i class="'.$brad_data['custom_iconfont']['prefix'].' '.$uploaded_icons[$icon_value].'"></i>';
		    }
		    else if($icon_type == 'ss-social'){
			   echo '<i class="ss-social-regular '.$ss_social[$icon_value].'"></i>';
		    }
		    else {
			   echo '<i class="'.$fa_icons[$icon_value].'"></i>';
		    }
			die();
			}
		    die(-1);
