...
if (!class_exists('WPBakeryShortCode')) {
    abstract class WPBakeryShortCode extends WPBakeryVisualComposerAbstract {
        ...
        protected function registerCss($param) {
            if(is_array($param)) {
                ...
                wp_register_style( $name, $param,  array( "brad-framework" ), time());
            }
        }
        ...
        public function output($atts, $content = null, $base = '') {
            ...
            if( empty($output) ) {
                ...
                if(function_exists($custom_output)) {
					...
					//Brad Shortcode helper to give index to the shortcode elements
					$helper = array(
					             'index' => brad_shortcodeHelper::$shortcode_index ,
								 'current' =>  brad_shortcodeHelper::find_tree_item(brad_shortcodeHelper::$shortcode_index),
								 'next' => brad_shortcodeHelper::find_tree_item(brad_shortcodeHelper::$shortcode_index,1),
								 'prev' => brad_shortcodeHelper::find_tree_item(brad_shortcodeHelper::$shortcode_index,-1));
								 // We need helper function only on section and double section
				    if(!empty($helper['current']['shortcode']) && !in_array( $helper['current']['shortcode'] , array('vc_section','vc_double_section','vc_button'))){
						$helper = false;
					}
					brad_shortcodeHelper::$shortcode_index++;			 
                    $output .=  $this->content( $this->atts, $content , $helper );
                }
                // After shortcode
    ...
    ---------------------------------------------------------- */
        public function getColumnControls($controls, $extended_css = '') {
            ...
            $controls_add = '';//' <a class="column_add" href="#" title="'.sprintf(__('Add to %s', "brad-framework"), strtolower($this->settings('name'))).'"></a>';
            $controls_edit = ' <a class="column_edit" href="#" title="'.sprintf(__('Edit %s', "brad-framework"), strtolower($this->settings('name'))).'"></a>';
            $controls_delete = ' <a class="column_clone" href="#" title="'.sprintf(__('Clone %s', "brad-framework"), strtolower($this->settings('name'))).'"></a> <a class="column_delete" href="#" title="'.sprintf(__('Delete %s', "brad-framework"), strtolower($this->settings('name'))).'"></a>';
       ...
       ---------------------------------------------------------- */
        ...
        public function singleParamHtmlHolder($param, $value) {
            ...
            if ( isset($param['holder']) === true && $param['holder'] == 'icon' ) {
				$output .= '<span class="wpb_vc_icon wpb-icon-prefix ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '"><i class="'.$value.'"></i></span>';
			}else if ( isset($param['holder']) === true && $param['holder'] !== 'hidden' ) {
        ...
        protected function outputTitle($title) {
            return  '<h4 class="wpb_element_title"><span>'.$title.'</span></h4>';
        }
    ...
    abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
        ...
        public function contentAdmin($atts,  $content) {
            ...
            if ( isset($this->settings['params']) ) {
                ...
                $output .= '<div class="vc_span12 wpb_edit_form_elements"><h2>'.__('Edit', "brad-framework").' ' .$this->settings['name'].'</h2>';
                ...
                $output .= '<div class="edit_form_actions"><a href="#" class="wpb_save_edit_form button-primary">'. __('Save', "brad-framework") .'</a></div>';
        ...
        protected function singleParamEditHolder($param, $param_value) {
			...
			$output .= '<div class="wpb_element_label vc_span3">';
            $output .= (isset($param['heading'])) ? $param['heading'] : '';//vc_span3
			$output .= '</div>';

            $output .= '<div class="edit_form_line vc_span9">';//vc_span9
            $output .= $this->singleParamEditForm($param, $param_value);
            $output .= (isset($param['description'])) ? '<span class="description clear">'.$param['description'].'</span>' : '';
			$output .= '</div>';
        ...
        protected function singleParamEditForm($param, $param_value) {
            ...
            // Checkboxes with post types
                ...
                foreach ( $values as $label => $v ) {
                    ...
                    $param_line .= ' <input id="'. $param['param_name'] . '-' . $v .'" value="' . $v . '" class="'.$param['param_name'].' '.$param['type'].'" type="checkbox" name="'.$param['param_name'].'"'.$checked.'> ' . $label;
                }
            }
            ...
            // Big Regular textarea
                ...
                // $param_value = __($param_value, "brad-framework");
            ...
            // Attach images
                ...
                //$param_line .= '<a class="button gallery_widget_add_images" href="#" title="'.__('Add images', "brad-framework").'">'.__('Add images', "brad-framework").'</a>';
                ...
                $param_line .= '</div>';
                $param_line .= '<a class="gallery_widget_add_images" href="#" title="'.__('Add images', "brad-framework").'">'.__('Add images', "brad-framework").'</a>';//class: button
                ...
                $param_line .= '<input type="hidden" class="wpb_vc_param_value gallery_widget_attached_images_ids '.$param['param_name'].' '.$param['type'].'" name="'.$param['param_name'].'" value="'.$param_value.'"/>';
                //$param_line .= '<a class="button gallery_widget_add_images" href="#" use-single="true" title="'.__('Add image', "brad-framework").'">'.__('Add image', "brad-framework").'</a>';
                $param_line .= '<div class="gallery_widget_attached_images">';
                $param_line .= '<ul class="gallery_widget_attached_images_list">';
                $param_line .= ($param_value != '') ? fieldAttachedImages(explode(",", $param_value)) : '';
                $param_line .= '</ul>';
                ...
                // $param_line .= siteAttachedImages(explode(",", $param_value));
                $param_line .= '</div>';
                $param_line .= '<a class="gallery_widget_add_images" href="#" use-single="true" title="'.__('Add image', "brad-framework").'">'.__('Add image', "brad-framework").'</a>';//class: button
                //$param_line .= '<div class="wpb_clear"></div>';
            }       //
        ...
        protected function getTinyHtmlTextArea($param = array(), $param_value) {
            ...
            //$upload_media_btns = '<div class="wpb_media-buttons hide-if-no-js"> '.__('Upload/Insert').' <a title="'.__('Add an Image').'" class="wpb_insert-image" href="#"><img alt="'.__('Add an Image').'" src="'.home_url().'/wp-admin/images/media-button-image.gif"></a> <a class="wpb_switch-editors" title="'.__('Switch Editors').'" href="#">HTML mode</a></div>';
                    ...
                    if ( function_exists('wp_editor') ) {
                        $default_content = __($param_value, "brad-framework");
                        $output_value = '';
                        // WP 3.3+
                        ob_start();
...
if (!class_exists('WPBakeryShortCode_Settings')) {
    class WPBakeryShortCode_Settings extends WPBakeryShortCode_UniversalAdmin {
        ...
        public function content( $atts, $content = null , $helper = false  ) {
            return '';
        }
        ...
        public function contentAdmin($atts, $content) {
            ...
            if ( isset($this->settings['params']) ) {
                ...
                $output .= '<div class="vc_span12 wpb_edit_form_elements"><h2 style="display: none;">'.__('Edit', "brad-framework").' ' .__($this->settings['name'], "brad-framework").'</h2>';
