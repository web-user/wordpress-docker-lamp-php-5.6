...
function wpb_getImageBySize ( $params = array( 'post_id' => NULL, 'attach_id' => NULL, 'thumb_size' => '' ) ) {
	...
	{
		$thumbnail = wp_get_attachment_image( $attach_id , "" );
	}
    elseif ( in_array($thumb_size, array('thumbnail', 'mini', 'medium', 'thumb-large-fullwidth','thumb-large' , 'thumb-medium-fullwidth','thumb-medium', 'thumb-normal-fullwidth','thumb-normal' ) )) {
    ...
    function siteAttachedImages($att_ids = array()) {
        ...
        foreach ( $media_images as $image_post ) {
            ...
            $output .= '<li'.$class.'>
						...
						<span class="img-added">'. __('Added', "brad-framework") .'</span>
					</li>';
        }
...
function vc_convert_shortcode($m) {
    ...
    if($tag == 'vc_section' || $tag == 'vc_double_section' || $tag == 'vc_row') return $output;
    // Start
